...
np.random.seed(17411) # for reproducibility
...
X, labels = load_data(prefix+'train.csv', train=True)
X, scaler = preprocess_data(X)
y, encoder = preprocess_labels(labels)

X_test, ids = load_data(prefix+'test.csv', train=False)
...
model.fit(X, y, nb_epoch=30, batch_size=32,validation_split=0.0)

print("Generating submission...")

proba = model.predict_proba(X_test)
make_submission(proba, ids, encoder, fname=prefix+'NN_SGD3.csv')
